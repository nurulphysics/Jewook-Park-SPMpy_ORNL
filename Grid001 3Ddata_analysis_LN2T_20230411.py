...
hv_bias_mV_slicing(grid_LDOS, ch = 'LDOS_fb',frame_width=400)#.opts(clim = (0,1.5E-10))
...
bound_box = hv.streams.BoundsXY(source = bbox_points,
                                bounds=(0,0,0,0))
dmap*bbox_points.opts(clim = (0,1E-11))*bbox_points
...
LDOS_fb_area1_df =  bbox_1.LDOS_fb.mean(['X','Y']).to_dataframe()
LDOS_fb_area2_df =  bbox_2.LDOS_fb.mean(['X','Y']).to_dataframe()
LDOS_fb_area_df = pd.concat( [LDOS_fb_area1_df,LDOS_fb_area2_df], axis= 1)
LDOS_fb_area_df.columns = ['area1','area2']
sns.lineplot(LDOS_fb_area_df)
plt.show()

# +
## comparision with STS vs Topo
...
# +
# set slicing bias_mV index
bias_mV_slices= [0, 50,100,150, 200, 250,300, 350,400][::-1]
bias_mV_slices_v = grid_LDOS.bias_mV.isel(bias_mV = bias_mV_slices).values.astype(int)

g = isns.ImageGrid(grid_LDOS.LDOS_fb.values, cbar=False, height=2, col_wrap=3, slices= bias_mV_slices , cmap="bwr")
col_wrap=3
...
grid_LDOS_th=th_mean_roi_label_2D_xr(filter_gaussian_xr(grid_LDOS), bias_mV_th= -200, threshold_flip= True)
...
grid_LDOS_rot= grid_LDOS_rot_bbox
...
rec_x0, rec_y0 = 3.5E-9,6.7E-9
rec_width,rec_height = 3E-9, 0.1E-9
...
# +
grid_LDOS_sg = savgolFilter_xr(grid_LDOS_zm, window_length=31, polyorder=3)
grid_LDOS_1diff =  grid_LDOS_sg.differentiate('bias_mV')
grid_LDOS_1diff_sg = savgolFilter_xr(grid_LDOS_1diff, window_length=31, polyorder=3)
grid_LDOS_2diff =  grid_LDOS_1diff_sg.differentiate('bias_mV')
grid_LDOS_2diff_sg = savgolFilter_xr(grid_LDOS_2diff, window_length=31, polyorder=3)
...
# +
grid_LDOS_sg_pk  = grid3D_line_avg_pks( grid_LDOS_sg,ch_l_name ='LDOS_fb', average_in= 'Y',distance = 100, threshold = 1E-13) 

grid_LDOS_sg_pk

# -

grid_LDOS_sg_pk_slct, grid_LDOS_sg_df, grid_LDOS_sg_pk_df, fig = grid_lineNpks_offset(grid_LDOS_sg_pk,ch_l_name ='LDOS_fb', plot_y_offset= 2E-13, legend_title = "X (nm)")#,peak_LIX_min = 1E-12)
plt.show()
