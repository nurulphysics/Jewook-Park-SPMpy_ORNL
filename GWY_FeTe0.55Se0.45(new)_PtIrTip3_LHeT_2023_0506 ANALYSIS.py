...
gwy_xr_crop = crop_square(image_xr = gwy_xr, ch_name= 'z_f')
#gwy_xr_crop = crop_square(image_xr = gwy_xr, ch= 'z_f_C')
...
# +
#gwy_xr.z_f.plot()
gwy_xr_zm1 =  gwy_xr.where(gwy_xr.Y>1.14E-7, drop = True).where(gwy_xr.X>1.14E-7, drop = True)
gwy_xr_zm2 =  gwy_xr.where(
    gwy_xr.Y>1.05E-7, drop = True).where(
    gwy_xr.Y<1.15E-7, drop = True).where(
    gwy_xr.X>1.05E-7, drop = True).where(
    gwy_xr.X<1.15E-7, drop = True)
...
# +
gwyxr_fft_zm = fft_masking_near_q0only (gwyxr_fft, ch_name = 'z_f_fft', filtering_ratio=[0.1, 2])
...
# +
#xrdata_plot_r_q_space(gwy_xr,ch_name= 'z_f',figsize = (5,2.5), distance_to_refq0 = 10, peak_detection_min_distance  = 10 )
#xrdata_plot_r_q_space(gwy_xr,ch_name= 'LIX_f',figsize = (5,2.5),distance_to_refq0 = 10, peak_detection_min_distance  = 10)

#xrdata_plot_r_q_space(gwy_xr,ch_name= 'z_f_C',figsize = (5,2.5), distance_to_refq0 = 10, peak_detection_min_distance  = 10 )
#xrdata_plot_r_q_space(gwy_xr,ch_name= 'LIX_f_C',figsize = (5,2.5),distance_to_refq0 = 10, peak_detection_min_distance  = 10)


# +
# if gwy data is  not "Corrected" 
filtering_ratio=[0.10, 2]
...
# +
#gwy_xr.z_f.plot()
gwy_xr_zm1 =  gwy_xr.where(gwy_xr.Y>1.08E-7, drop = True).where(gwy_xr.X>1.08E-7, drop = True)
gwy_xr_zm2 =  gwy_xr.where(
    gwy_xr.Y>1.000E-7, drop = True).where(
    gwy_xr.Y<1.05E-7, drop = True).where(
    gwy_xr.X>1.150E-7, drop = True).where(
    gwy_xr.X<1.20E-7, drop = True)


gwy_xr_zm2

xrdata_plot_r_q_space_q(gwy_xr_zm2,distance_to_refq0 = 1, peak_detection_min_distance  = 1, add_guidlines = False)


# +
# if gwy data is  not "Corrected" 
filtering_ratio=[0.10, 2]
xrdata_plot_r_q_space_q3(gwy_xr_zm2,ch_name= 'z_f',
                        figsize = (6,3),filtering_ratio=filtering_ratio,
                        add_guidlines = False)
xrdata_plot_r_q_space_q3(gwy_xr_zm2,ch_name= 'LIX_f',
                        figsize = (6,3),filtering_ratio=filtering_ratio,
                        add_guidlines = False )
