...
hv_bias_mV_slicing(grid_LDOS, ch = 'LDOS_fb')#.opts(clim = (0,1.5E-10))
...
bbox_2, _ = hv_bbox_avg(grid_LDOS, bound_box= bound_box, ch ='LDOS_fb')
...
# +
sns.scatterplot(x = 'bias_mV', y = 'LDOS_fb' , data  = bbox_1.LDOS_fb.mean(['X','Y']).to_dataframe(), label ='area1', color = 'y')
sns.scatterplot(x = 'bias_mV', y = 'LDOS_fb' , data  = bbox_2.LDOS_fb.mean(['X','Y']).to_dataframe(), label ='area2', color = 'r' )
...
#threshold_otsu_xr(grid_LDOS.isel(bias_mV=0), treshold_flip = False).LDOS_fb.plot()
#plt.show()
grid_LDOS_th= grid_LDOS.copy()
grid_LDOS_th['LDOS_th'] = threshold_otsu_xr(grid_LDOS.isel(bias_mV=0), treshold_flip = False).LDOS_fb

grid_LDOS

grid_LDOS['LDOS_th'].values = threshold_otsu_xr(grid_LDOS.isel(bias_mV=0), treshold_flip = False).LDOS_fb.values
#grid_LDOS['LDOS_th_label'] = measure.label(grid_LDOS.LDOS_th.values)

grid_LDOS.LDOS_th_label.plot()
plt.show()

roi_0 =  threshold_otsu_xr( grid_LDOS.isel(bias_mV=0)).LDOS_fb.values
from skimage import data, filters, measure, morphology
roi_0_lable = measure.label(roi_0)
isns.imshow(roi_0_lable)
plt.show()
...
# +
ds0 = hv.Dataset(grid_LDOS.isel(bias_mV=0))
...
(im *xr_data_channel_hv_image )

# +
# plot 

im = ds.to(hv.Image, ['X', 'Y'], dynamic=True)
(im *xr_data_channel_hv_image* polys + dmap * hlines).opts(
...
im = ds.to(hv.Image, ['X', 'Y'], dynamic=True).add_tools(BoxEditTool())
(im * polys).opts(
    opts.Curve(width=400, framewise=True), 
    opts.Polygons(fill_alpha=0.2, line_color='white'), 
    opts.VLine(color='black')) 
