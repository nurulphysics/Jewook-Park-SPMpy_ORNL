...
#
# ## <font color= red > Magnetic field 0.01 T (Z)   </font>
#
...
sns.lineplot( data = grid_3D_gap.zerobiasconductance.to_dataframe(), x ='X', y =  'zerobiasconductance', marker = 'o')
#sns.scatterplot( data = grid_3D_gap.zerobiasconductance.to_dataframe(), x ='X', y =  'zerobiasconductance')
...
# +
#grid_LDOS_sg.LDOS_fb.sel(Y=0, method ="nearest").plot.imshow(robust= True)

fig,axs  = plt.subplots (1,1, figsize  = (6,6) )
g = isns.imshow (grid_LDOS_sg.LDOS_fb.sel(Y=0, method ="nearest"), aspect =0.1 , robust= True, perc=(10,90),cbar =False, ax  = axs )# robust= True, percentiles=(5, 90), cbar =False, 
g.set_aspect (5)
plt.show()
...
hv_bias_mV_slicing(grid_LDOS, ch = 'LDOS_fb',frame_width=300).opts(clim = (0.2E-9,0.8E-9)) # adjust cbar limit

# ####  1.5.2. Y or X slicing 

#hv_XY_slicing(grid_LDOS, ch = 'LDOS_fb',slicing= 'Y')#.opts(clim=(0, 8E-10)) #
hv_XY_slicing(grid_LDOS, ch = 'LDOS_fb',slicing= 'X',frame_width=300).opts(clim=(0, 1.5E-9)) #
...
plot_XYslice_w_LDOS(grid_LDOS, sliderX= sliderX, sliderY= sliderY, ch = 'LDOS_fb',slicing_bias_mV = 0.2)
# -
...
bias_mV_slices = np.arange(-1.50,1.51,0.3)
...
np.savetxt('grid_LDOS_a_b_LDOS_fb.txt',grid_LDOS_a_b.LDOS_fb.values)
...
grid_LDOS_sg= savgolFilter_xr(grid_LDOS_rot, window_length=31, polyorder=2)
...
bound_box = hv.streams.BoundsXY(source = bbox_points,
                                bounds=(0,0,0,0))
dmap.opts(clim = (0,1.6E-9))*bbox_points
#dmap*bbox_points
...
# Choose peak labels
###############
grid_LDOS_bbox_pk_df_choose = grid_LDOS_bbox_pk_df [(grid_LDOS_bbox_pk_df.y_kmeans  ==1)
