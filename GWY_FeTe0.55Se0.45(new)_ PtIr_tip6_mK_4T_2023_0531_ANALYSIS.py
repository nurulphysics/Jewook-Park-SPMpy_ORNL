...
# * 2023 0531 FeTe0.55Se0.45 PtIr tip6 mK Jewook
# ## Measurement temp
...
# ## **Tip: PtIr Tip:  <font color= Blue, font size="5" > Spin-Avergaged </font>  tip**
# ## <font color= red >  Magnetic field 4T (Z)   </font>
...
folder_path = r'C:\Users\gkp\OneDrive - Oak Ridge National Laboratory\0_mK STM DATA\2023\0531 FeTe0.55Se0.45(old)_PtIr_tip6_mK_Mag_Field_Jewook - magnet heater was off'
...
# +
## Choose target file 
gwy_analyze_filename = gwy_files[gwy_files.file_name.str.contains(isin_filename)].iloc[0].file_name
...
gwy_xr_zm2 =  gwy_xr.where(
    gwy_xr.Y>0.8E-7, drop = True).where(
    gwy_xr.Y<1.05E-7, drop = True).where(
    gwy_xr.X>0.8E-7, drop = True).where(
    gwy_xr.X<1.05E-7, drop = True)
