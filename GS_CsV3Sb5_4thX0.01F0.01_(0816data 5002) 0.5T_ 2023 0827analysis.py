...
# line data
grid_xr = grid2xr(files_df[files_df.type=='3ds'].file_name.iloc[16])
grid_xr
...
grid_xr_crop = grid_xr#.where(grid_xr.Y <1.05E-7, drop = True)
...
grid_3D_gap.zerobiasconductance.plot(robust = True)
...
hv_bias_mV_slicing(grid_LDOS, ch = 'LDOS_fb',frame_width=300).opts(clim = (0.0001E-9,0.5E-9)) # adjust cbar limit
...
hv_XY_slicing(grid_LDOS, ch = 'LDOS_fb',slicing= 'Y',frame_width=300).opts(clim=(0, 0.5E-9)) # check low intensity area
...
bias_mV_slices = np.arange(-1.8,1.9,0.4)
