...
isns.imshow (grid_topo_smth.topography, ax =axs,cbar= False, cmap = "copper")
...
label6_df = grid_LDOS.LDOS_fb.where( label6, drop= True).to_dataframe().dropna()
label7_df = grid_LDOS.LDOS_fb.where( label7, drop= True).to_dataframe().dropna()
label8_df = grid_LDOS.LDOS_fb.where( label8, drop= True).to_dataframe().dropna()
...
sns.lineplot(data=concatenated_LDOS, x='bias_mV', y='LDOS_fb', hue='relative position', palette = 'inferno_r')b
