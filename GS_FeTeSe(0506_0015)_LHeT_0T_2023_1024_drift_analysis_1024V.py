...
#grid_topo_crop.isel(crop=0).dropna(dim="X", how="all").dropna(dim="Y", how="all")
twoD_FFT_xr(grid_topo_crop.isel(crop=1).dropna(dim="X", how="all").dropna(dim="Y", how="all"))

grid_topo_crop.attrs['ref_a0']
...
# +
# prepare targe area for peak detection 


grid_topo_crop_fft = twoD_FFT_xr(grid_topo_crop.isel(crop=0).dropna(dim="X", how="all").dropna(dim="Y", how="all").drop_vars(['u_xr','v_xr']))
#grid_topo_crop_fft
# Generate 2D data using the formula sqrt(x^2 + y^2)
distancefromcenter = np.sqrt(np.power(grid_topo_crop_fft.freq_X, 2) + np.power(grid_topo_crop_fft.freq_Y, 2))
tangent = np.arctan( grid_topo_crop_fft.freq_Y/grid_topo_crop_fft.freq_X)

#distancefromcenter
grid_topo_crop_fft['distancefromcenter']=distancefromcenter
grid_topo_crop_fft['tangent']=tangent

# -

grid_topo_crop_fft.tangent.plot()

# +
grid_topo_crop_fft_peak_area= grid_topo_crop_fft.where(
    (grid_topo_crop_fft.distancefromcenter> grid_topo_crop_fft.ref_q0 *0.5)&
    (grid_topo_crop_fft.distancefromcenter< grid_topo_crop_fft.ref_q0 *2)&
    (grid_topo_crop_fft.tangent> 10*math.pi/180)&
    (grid_topo_crop_fft.tangent< 80*math.pi/180)&
    (grid_topo_crop_fft.freq_X>0)&
    (grid_topo_crop_fft.freq_Y>0)).topography_fft
...
fft_peak = skimage.feature.peak_local_max(np.log(grid_topo_crop_fft_peak_area.fillna(0)).values, min_distance= 2)[0]
# point 찾기 위해서  fillna(0) 와 np.log가 필요함. 
fft_peak
#p.log(grid_topo_crop_fft_peak_area.fillna(0)).values

# +
v_xr = fft_peak[0]
u_xr = fft_peak[1]

(u,v) vector filed map 
