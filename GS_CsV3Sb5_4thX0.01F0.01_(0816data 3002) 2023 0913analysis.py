...
grid_xr_drft_y_rot = grid_xr_drft_y_rot.where(
    grid_xr_drft_y_rot.X >1.8E-9, drop = True).where(
    grid_xr_drft_y_rot.X <3.80E-9, drop = True).where(
    grid_xr_drft_y_rot.Y >1.8E-9, drop = True).where(
    grid_xr_drft_y_rot.Y <3.80E-9, drop = True)
...
for axes_i  in range( len(bias_mV_slices)):
    ...
    g.axes[int((axes_i)/col_wrap)][axes_i%col_wrap].set_title(str(bias_mV_slices_v[axes_i].round(1))+' mV')
plt.tight_layout()
plt.show()
...
#grid_LDOS_df = grid_LDOS.where(abs(grid_LDOS.bias_mV)<1.0, drop = True) .to_dataframe().unstack()
...
isns.imshow (grid_LDOS.LDOS_fb.where(grid_LDOS.bias_mV ==0, drop = True ), robust = True,cmap = 'Blues',ax=axs)
axs.set_title('Zero bias conductance map')#, fontsize = 'medium')
plt.show()
...
fig,axs = plt.subplots(1,1, figsize = (5,5))
isns.imshow( grid_LDOS_a_b.LDOS_fb, robust = True, ax = axs, cmap = 'bwr')
axs.set_title (  r'$\Delta$ ' + 'LDOS \n' + '(' +str(bias_mv_a) + ' mV - '  +str(bias_mV_b) + ' mV )')#, fontsize = 'medium')
plt.show()
...
fig,axs = plt.subplots(1,1, figsize = (5,5))
isns.imshow(grid_LDOS_ref, robust = True, cmap = 'bwr',ax = axs)
#g.set_title ('LDOS average between ' +str(LDOS_range_mV), fontsize = 'medium')

axs.set_title ('LDOS map \n at coherent peak' +' ('+ str(np.array(LDOS_range_mV).mean())+' mV)')#, fontsize = 'medium')
plt.show()



# +
grid_LDOS_th= th_multiotsu_roi_label_2D_xr(grid_LDOS, bias_mV_th = 0.0, multiclasses = 6)
...
grid_LDOS_th_df = grid_LDOS_th.to_dataframe().reset_index()
# df로 만들고, reset
sns.lineplot(data= grid_LDOS_th_df, x ='bias_mV', y = 'LDOS_fb', hue = 'LDOS_fb_th') 
#grid_LDOS_th.where (grid_LDOS_th.LDOS_fb_th == 1, drop = True) 
...
LDOS_fb_1_df = grid_LDOS_th.LDOS_fb.where( grid_LDOS_th.LDOS_fb_th_label ==2 ).to_dataframe()
...
isns.imshow (grid_LDOS_th.LDOS_fb_th!=0, ax = ax[1]) 
...
image_max = ndi.maximum_filter(im_ivt, size=1, mode='constant')
...
for idx in coordinates:
    ...
    for i in range(avg_line_length-1):
        #print (i)
        #grid_topo_smth.label[idx[0]+i-5, idx[1]-1] = i+1
        grid_topo_smth.label[idx[0]+i-5, idx[1]] = i+1
        #grid_topo_smth.label[idx[0]+i-5, idx[1]+1] = i+1
fig,axs = plt.subplots(1,1, figsize = (5,5))

isns.imshow(grid_topo_smth.topography, cmap = 'gray', ax = axs) 

isns.imshow(grid_topo_smth.label.where(grid_topo_smth.label!=0), cmap = 'viridis', ax = axs, alpha =0.8)
axs.plot(coordinates[:, 1], coordinates[:, 0], marker='o', markersize=8, linestyle ='',  color='red',)
...
isns.imshow (grid_topo_smth.label.where (grid_topo_smth.label!=0), cmap = 'viridis', ax =axs)
plt.show()
...
for i, data in enumerate(LDOS_label):
    column_name = f'Label_{i}'  # 새로운 열 이름 생성
    print(column_name)
...
#grid_topo_smth.label.max()
grid_topo_smth_label_mean = grid_topo_smth[['topography','label']].to_dataframe().groupby('label').mean().reset_index()
grid_topo_smth_label_mean['position'] = grid_topo_smth_label_mean.label

grid_topo_smth_label_mean

# +
fig, axs = plt.subplots(2,1, figsize = (5,5))
isns.imshow(LDOS_label_df, cbar = False, cmap = 'bwr', ax =  axs[0], robust = True, perc= (5,98))
axs[0].set_aspect (0.02)
axs[0].set_ylabel('Bias (mV)')
#axs[0].set_xlabel('bias_mV')

sns.scatterplot(grid_topo_smth_label_mean,x='label', y='topography', hue = 'position', palette ='viridis')
axs[1].set_xlabel('points ')
axs[1].set_ylabel('z (a.u.)')

# 설명을 담은 bbox 추가
bbox_props = dict(boxstyle="round,pad=0.3", ec="black", lw=2, fc="white")
axs[1].text(2, 150, "Cs", ha="center", va="center", bbox=bbox_props, fontsize=12)
axs[1].axvline(x=2, color='gray', linestyle='--')
axs[1].text(6, 150, "Sb", ha="center", va="center", bbox=bbox_props, fontsize=12)
axs[1].axvline(x=6, color='gray', linestyle='--')
...
label9 = (grid_topo_smth.label == 9)
#label10 = (grid_topo_smth.label == 10)



expanded_mask_label0 = np.expand_dims(label0, axis=2)
...
expanded_mask_label9 = np.expand_dims(label9, axis=2)
#expanded_mask_label10 = np.expand_dims(label10, axis=2)
...
label0_df = grid_LDOS.LDOS_fb.where( expanded_mask_label0).to_dataframe().dropna()
...
label9_df = grid_LDOS.LDOS_fb.where( expanded_mask_label9).to_dataframe().dropna()
#label10_df = grid_LDOS.LDOS_fb.where( expanded_mask_label10).to_dataframe().dropna()


frames = [label0_df,label1_df, label2_df, label3_df, label4_df,label5_df,label6_df,label7_df,label8_df,label9_df]#,label10_df]
keys = ['0','1 ', '2(Cs)', '3', '4','5','6 (Sb)','7','8','9']#,'10']
...
# Matplotlib의 rcParams를 사용하여 글꼴 크기 설정
plt.rcParams.update({'font.size': 12})

fig,axs = plt.subplots(1,1, figsize = (6,5))


sns.lineplot(data=concatenated_LDOS, 
             x='bias_mV', y='LDOS_fb',
             hue='relative position', 
             palette = 'viridis',
             ax =axs)

axs.set_xlabel('Bias (mV)',fontsize='medium')
axs.set_ylabel('dI/dV (A)',fontsize='medium')
axs.set_xlim(-1.6,1.6)
axs.legend(loc='lower right',fontsize='medium'),# bbox_to_anchor=(1,1))
plt.show()
...
# 설명을 담은 bbox 추가
bbox_props = dict(boxstyle="round,pad=0.3", ec="black", lw=2, fc="white")
axs[1].text(1, 150, "Cs", ha="center", va="center", bbox=bbox_props, fontsize=12)
axs[1].axvline(x=1, color='gray', linestyle='--')
axs[1].text(6, 150, "V - Sb - V", ha="center", va="center", bbox=bbox_props, fontsize=12)
axs[1].axvline(x=6, color='gray', linestyle='--')
...
sns.scatterplot(grid_topo_smth_label_mean,x='label', y='topography', hue = 'position', palette ='inferno')
axs[1].set_xlabel('points ')
axs[1].set_ylabel('z (a.u.)')
...
frames = [label0_df,label1_df, label2_df, label3_df, label4_df,label5_df,label6_df,label7_df,label8_df,label9_df,label10_df]
keys = ['0','1 (Cs)', '2', '3', '4','5','6 (Sb)','7','8','9','10']
...
# +
label0_mean_df = label0_df.groupby('bias_mV').mean()
...
label9_mean_df = label9_df.groupby('bias_mV').mean()
label10_mean_df = label10_df.groupby('bias_mV').mean()




frames_mean = [label0_mean_df,label1_mean_df, label2_mean_df, label3_mean_df, label4_mean_df,label5_mean_df,label6_mean_df,label7_mean_df,label8_mean_df]
keys = ['0','1 (Cs)', '2', '3', '4','5','6 (V)','7','8','9','10']

concatenated_df = pd.concat(frames_mean, keys=keys)


g = isns.imshow( concatenated_df.unstack().T, robust = True, perc = (5,100), cbar  = False, cmap = 'bwr')
g.set_aspect(0.01)
plt.show()
...
# +
#bound_box = BoundsXY(bounds=(2.200474716202268e-09, 9.525022949146754e-10, 2.4604662506449924e-09, 4.089521693134486e-09))
# -
...
# +
#del grid_LDOS_bbox.attrs['threshold']
# attrs 가운데 
#grid_LDOS_bbox.to_netcdf('bbox_0816data3002_20230907DA.nc')
...
ax.legend(handles2,   labels2, title = legend_title,loc='upper right', bbox_to_anchor=(1.3, 0.4))
