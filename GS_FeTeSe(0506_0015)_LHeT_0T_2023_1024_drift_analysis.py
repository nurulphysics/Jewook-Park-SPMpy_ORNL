...
#     * Cleaving: @ UHV Loadlock chamber, Room temp(<5E-10Torr))
# ## **Tip** 
# *  PtIr <font color= Blue, font size="5" > normal metal tip   </font>  tip**
...
folder_path = r'C:\Users\gkp\OneDrive - Oak Ridge National Laboratory\0_mK STM DATA\2023\0506 FeTe0.55Se0.45_new_batch1_PtIrTip3_LHeT_Jewook'
...
grid_topo_crop = crop_xrdata(grid_topo, crop_size = 36)
grid_topo_crop
# -

grid_topo_crop.crop

grid_topo_crop.isel(crop=1)

grid_topo_crop

grid_topo_crop.attrs['ref_a0']=0.38E-9
grid_topo_crop.attrs['ref_q0']=1/grid_topo_crop.attrs['ref_a0']
grid_topo_crop
# attributes 추가 
...
grid_topo_crop = crop_for_LFFT_xrdata(grid_topo, crop_size= 32)
...
def local_FFT_A_uv_xr(grid_topo_crop, ref_a0=0.38E-9, estimated_q0_range=(0.5, 2), angle_margin=20, peak_angle_deg=45, quadrant=None):
    """
    Perform local FFT analysis on a cropped grid dataset and estimate A_u_xr and A_v_xr values.

    Parameters:
        grid_topo_crop (xarray.Dataset): Input dataset with cropped grid data.
        ref_a0 (float, optional): Reference value for a0. Default is 0.38E-9.
        estimated_q0_range (tuple, optional): Estimated q0 range (min, max). Default is (0.5, 2).
        angle_margin (int, optional): Angle margin in degrees for peak detection. Default is 20.
        peak_angle_deg (int, optional): Expected peak angle in degrees. Default is 45.
        quadrant (int, optional): Quadrant for peak detection (1, 2, 3, or 4).

    Returns:
        xarray.Dataset: Processed dataset with A_u_xr and A_v_xr values estimated and added.
    """

    # Set reference values for a0 and q0
    grid_topo_crop.attrs['ref_a0'] = ref_a0
    grid_topo_crop.attrs['ref_q0'] = 1 / ref_a0

    # Determine the quadrant if not specified
    if quadrant is None:
        if 0 <= (peak_angle_deg % 360) < 90:
            quadrant = 1
        elif 90 <= (peak_angle_deg % 360) < 180:
            quadrant = 2
        elif 180 <= (peak_angle_deg % 360) < 270:
            quadrant = 3
        elif 270 <= (peak_angle_deg % 360) < 360:
            quadrant = 4
        else:
            raise ValueError("Invalid quadrant. Set quadrant to 1, 2, 3, or 4.")

    for crop_i in range(len(grid_topo_crop.crop)):
        # Check if A_u_xr and A_v_xr already exist, if not, initialize them
        if 'A_u_xr' not in grid_topo_crop.data_vars:
            grid_topo_crop['A_u_xr'] = grid_topo_crop.topography.mean(dim=["X", "Y"]).copy()
        if 'A_v_xr' not in grid_topo_crop.data_vars:
            grid_topo_crop['A_v_xr'] = grid_topo_crop.topography.mean(dim=["X", "Y"]).copy()
        else: pass
            
        # Drop variables A_u_xr, A_v_xr 
        # there is no B_u_xr, and B_v_xr            
        if 'B_u_xr' in grid_topo_crop.data_vars  or 'B_u_xr' in grid_topo_crop.data_vars :
        # Drop variables A_u_xr, A_v_xr, B_u_xr, and B_v_xr
            data_vars_to_drop = ['A_u_xr', 'A_v_xr', 'B_u_xr', 'B_v_xr']
        else: 
            data_vars_to_drop = ['A_u_xr', 'A_v_xr']

            
        print (data_vars_to_drop)
        # Perform 2D FFT on the cropped data
        cropped_data_fft = twoD_FFT_xr(grid_topo_crop.isel(crop=crop_i).dropna(dim="X", how="all").dropna(dim="Y", how="all").drop_vars(data_vars_to_drop))

        # Calculate distance from the center and tangent values
        distancefromcenter = np.sqrt(np.power(cropped_data_fft.freq_X, 2) + np.power(cropped_data_fft.freq_Y, 2))
        tangent = np.arctan(cropped_data_fft.freq_Y / cropped_data_fft.freq_X)

        cropped_data_fft['distancefromcenter'] = distancefromcenter
        cropped_data_fft['tangent'] = tangent

        peak_area_condition0 = (cropped_data_fft.distancefromcenter > grid_topo_crop.ref_q0 * estimated_q0_range[0]
                               )&(cropped_data_fft.distancefromcenter < grid_topo_crop.ref_q0 * estimated_q0_range[1]
                                 )&(cropped_data_fft.tangent > (peak_angle_deg - angle_margin) * math.pi / 180
                                   ) &(cropped_data_fft.tangent < (peak_angle_deg + angle_margin) * math.pi / 180) 

        # Define the range for peak detection based on the quadrant
        if quadrant == 1:
            condition = (peak_area_condition0 &
                         (cropped_data_fft.freq_X > 0) &
                         (cropped_data_fft.freq_Y > 0))
        elif quadrant == 2:
            condition = (peak_area_condition0 &
                         (cropped_data_fft.freq_X < 0) &
                         (cropped_data_fft.freq_Y > 0))
        ...
        # Continue with peak detection using the condition
        cropped_data_fft_area = cropped_data_fft.where(condition).topography_fft

        # Perform peak detection
        fft_peak = skimage.feature.peak_local_max(np.log(cropped_data_fft_area.fillna(0)).values, min_distance=2)

        # Update A_u_xr and A_v_xr values based on peak detection
        if np.size(fft_peak) != 0:
            grid_topo_crop.A_v_xr.loc[dict(crop=crop_i)] = fft_peak[0][0]
            grid_topo_crop.A_u_xr.loc[dict(crop=crop_i)] = fft_peak[0][1]
        else:
            grid_topo_crop.A_v_xr.loc[dict(crop=crop_i)] = np.nan
            grid_topo_crop.A_u_xr.loc[dict(crop=crop_i)] = np.nan

    return grid_topo_crop
...
def local_FFT_B_uv_xr(grid_topo_crop, ref_a0=0.38E-9, estimated_q0_range=(0.5, 2), angle_margin=20, peak_angle_deg=135, quadrant=None):
    """
    Perform local FFT analysis on a cropped grid dataset and estimate B_u_xr and B_v_xr values.
    ...
    Parameters:
        ...
        estimated_q0_range (tuple, optional): Estimated q0 range (min, max). Default is (0.5, 2).
        angle_margin (int, optional): Angle margin in degrees for peak detection. Default is 20.
        peak_angle_deg (int, optional): Expected peak angle in degrees. Default is 135.
        quadrant (int, optional): Quadrant for peak detection (1, 2, 3, or 4).

    Returns:
        xarray.Dataset: Processed dataset with B_u_xr and B_v_xr values estimated and added.
    """
    # Set reference values for a0 and q0
    grid_topo_crop.attrs['ref_a0'] = ref_a0
    grid_topo_crop.attrs['ref_q0'] = 1 / ref_a0
    ...
    for crop_i in range(len(grid_topo_crop.crop)):
        # Check if B_u_xr and B_v_xr already exist, if not, initialize them
        if 'B_u_xr' not in grid_topo_crop.data_vars:
            grid_topo_crop['B_u_xr'] = grid_topo_crop.topography.mean(dim=["X", "Y"]).copy()
        if 'B_v_xr' not in grid_topo_crop.data_vars:
            grid_topo_crop['B_v_xr'] = grid_topo_crop.topography.mean(dim=["X", "Y"]).copy()
        else:
            pass
        
        if 'A_u_xr' in grid_topo_crop.data_vars  or 'A_u_xr' in grid_topo_crop.data_vars :
        # Drop variables A_u_xr, A_v_xr, B_u_xr, and B_v_xr
            data_vars_to_drop = ['A_u_xr', 'A_v_xr', 'B_u_xr', 'B_v_xr']
        else: 
            data_vars_to_drop = ['B_u_xr', 'B_v_xr']

        #print (data_vars_to_drop)
        # Perform 2D FFT on the cropped data
        cropped_data_fft = twoD_FFT_xr(grid_topo_crop.isel(crop=crop_i).dropna(dim="X", how="all").dropna(dim="Y", how="all").drop_vars(data_vars_to_drop))
        print ("cropped_data_fft.shape = ", cropped_data_fft.dims)
        # Calculate distance from the center and tangent values
        distancefromcenter = np.sqrt(np.power(cropped_data_fft.freq_X, 2) + np.power(cropped_data_fft.freq_Y, 2))
        tangent = np.arctan(cropped_data_fft.freq_Y / cropped_data_fft.freq_X)
        ...
        # Update B_u_xr and B_v_xr values based on peak detection or closest point in q_0 range
        if np.size(fft_peak) != 0:
            grid_topo_crop.B_v_xr.loc[dict(crop=crop_i)] = fft_peak[0][0]
            grid_topo_crop.B_u_xr.loc[dict(crop=crop_i)] = fft_peak[0][1]
        else:
            # Calculate the desired tangent value
            tangent_deg = (135 * math.pi) / 180

            # Calculate the differences between the desired values and the available data
            tangent_diff = np.abs(cropped_data_fft.tangent - tangent_deg)
            q0_diff = np.abs(cropped_data_fft.distancefromcenter - cropped_data_fft.ref_q0)

            
            # Add the DataArrays to cropped_data_fft as new channels
            cropped_data_fft['tangent_diff'] = tangent_diff
            cropped_data_fft['q0_diff'] = q0_diff

            # Create an error metric as a combination of differences (you can define your own metric)
            error_metric = tangent_diff + q0_diff

            # Find the indices where the error metric is minimized
            min_indices = np.unravel_index(error_metric.argmin(), error_metric.shape)
            grid_topo_crop.B_v_xr.loc[dict(crop=crop_i)] = min_indices[0]
            grid_topo_crop.B_u_xr.loc[dict(crop=crop_i)] = min_indices[1]

    return grid_topo_crop

# -

grid_topo_crop_FFT_A_pk = local_FFT_A_uv_xr(grid_topo_crop)
grid_topo_crop_FFT_B_pk = local_FFT_B_uv_xr(grid_topo_crop_FFT_A_pk)


grid_topo_crop_FFT_B_pk==grid_topo_crop_FFT_A_pk
grid_topo_crop_FFT_A_pk

grid_topo_crop_FFT_A_pk.A_u_xr
grid_topo_crop_FFT_A_pk.A_v_xr
grid_topo_crop_FFT_B_pk.B_u_xr
grid_topo_crop_FFT_B_pk.B_v_xr
#grid_topo_crop_FFT_AB=

# +
grid_topo["A_u_xr"]= grid_topo.topography.copy()
grid_topo["A_v_xr"]= grid_topo.topography.copy()
grid_topo["B_u_xr"]= grid_topo.topography.copy()
grid_topo["B_v_xr"]= grid_topo.topography.copy()


grid_topo["A_u_xr"].values = grid_topo_crop_FFT_A_pk.A_u_xr.values.reshape(grid_topo.topography.values.shape)
grid_topo["A_v_xr"].values = grid_topo_crop_FFT_A_pk.A_v_xr.values.reshape(grid_topo.topography.values.shape)
grid_topo["A_uv_xr_modulous"] = np.sqrt(np.power(grid_topo.A_u_xr, 2) + np.power(grid_topo.A_v_xr, 2))
grid_topo["B_u_xr"].values = grid_topo_crop_FFT_B_pk.B_u_xr.values.reshape(grid_topo.topography.values.shape)
grid_topo["B_v_xr"].values = grid_topo_crop_FFT_B_pk.B_v_xr.values.reshape(grid_topo.topography.values.shape)
grid_topo["B_uv_xr_modulous"] = np.sqrt(np.power(grid_topo.B_u_xr, 2) + np.power(grid_topo.B_v_xr, 2))

# normalized 
grid_topo["A_u_xr_n"] = grid_topo.A_u_xr/grid_topo.A_uv_xr_modulous
grid_topo["A_v_xr_n"] = grid_topo.A_v_xr/grid_topo.A_uv_xr_modulous
grid_topo["B_u_xr_n"] = grid_topo.B_u_xr/grid_topo.B_uv_xr_modulous
grid_topo["B_v_xr_n"] = grid_topo.B_v_xr/grid_topo.B_uv_xr_modulous

grid_topo
# -

#grid_topo.A_uv_xr_modulous.plot()
grid_topo.B_uv_xr_modulous.plot()

grid_topo['A_v_xr_n_neg']= grid_topo.A_v_xr_n*-1
grid_topo['B_v_xr_n_neg']= grid_topo.B_v_xr_n*-1

# rotate 90deg & normalized 
#grid_topo.where(grid_topo.X<2E-9,drop=True).where(grid_topo.Y<2E-9,drop=True).plot.quiver(x="X", y="Y",u="A_u_xr_n",v="A_v_xr_n")
grid_topo.where(grid_topo.X<2E-9,drop=True).where(grid_topo.Y<2E-9,drop=True).plot.quiver(x="X", y="Y",u="B_u_xr_n",v="B_v_xr_n")

# +
fig,axs = plt.subplots(ncols = 2, nrows =1, figsize =(6,3))

axs[0].streamplot(x=grid_topo.X.values*1E9,
              y=grid_topo.Y.values*1E9,
              u=grid_topo.A_v_xr_n_neg.values,
              v=grid_topo.A_u_xr_n.values )

axs[1].streamplot(x=grid_topo.X.values*1E9,
              y=grid_topo.Y.values*1E9,
              u=grid_topo.B_v_xr_n_neg.values,
              v=grid_topo.B_u_xr_n.values )
...
# streamline plot --> x,y limit is not varying with XR data coords

# -

grid_topo_crop_fft_0 = twoD_FFT_xr(grid_topo_crop)
# build a crop_fft xr dataset first
# replace the topography_fft as a new one


grid_topo_crop_fft_0.freq_crop

# +
# Generate 2D data using the formula sqrt(x^2 + y^2)
distancefromcenter = np.sqrt(np.power(grid_topo_crop_fft_0.freq_X, 2) + np.power(grid_topo_crop_fft_0.freq_Y, 2))
tangent = np.arctan( grid_topo_crop_fft_0.freq_Y/grid_topo_crop_fft_0.freq_X)

#distancefromcenter
grid_topo_crop_fft_0['distancefromcenter']=distancefromcenter
grid_topo_crop_fft_0['tangent']=tangent
grid_topo_crop_fft_0
# -

grid_topo_crop_fft


twoD_FFT_xr(grid_topo_crop.isel(crop=200).dropna(dim="X", how="all").dropna(dim="Y", how="all")).topography_fft.plot()

grid_topo_crop_fft.freq_X[7]
...
grid_topo_crop['u_xr']= grid_topo_crop.topography.mean(dim= ["X","Y"]).copy()
grid_topo_crop['v_xr']= grid_topo_crop.topography.mean(dim= ["X","Y"]).copy()

# grid_topo_crop 의 pixel 별 (u,v) 공간 준비 
## crop하나씩 불러와서 
# FFT 

#twoD_FFT_xr(grid_topo_crop.isel(crop=0).dropna(dim="X", how="all").dropna(dim="Y", how="all").drop_vars(['u_xr','v_xr']))

crop_i=3

cropped_data_fft = twoD_FFT_xr(grid_topo_crop.isel(crop=crop_i).dropna(dim="X", how="all").dropna(dim="Y", how="all").drop_vars(['u_xr','v_xr']))
#cropped_data_fft.topography_fft.plot()
#cropped_data_fft.topography_fft.freq_Y

distancefromcenter = np.sqrt(np.power(cropped_data_fft.topography_fft.freq_X, 2) + np.power(cropped_data_fft.topography_fft.freq_Y, 2))
tangent = np.arctan( cropped_data_fft.topography_fft.freq_Y/cropped_data_fft.topography_fft.freq_X)
#filtering 기준 설정 
cropped_data_fft['distancefromcenter']=distancefromcenter
cropped_data_fft['tangent']=tangent
#  original data pixel 이 아니라 cropped_data_fft 에 대입, 
# -

# peak searching range setup 
grid_topo_crop_fft_peak_area= cropped_data_fft.where(
    (cropped_data_fft.distancefromcenter> grid_topo_crop_fft.ref_q0 *0.5)&
    (cropped_data_fft.distancefromcenter< grid_topo_crop_fft.ref_q0 *2)&
    (cropped_data_fft.tangent> 15*math.pi/180)&
    (cropped_data_fft.tangent< 75*math.pi/180)&
    (cropped_data_fft.freq_X>0)&
    (cropped_data_fft.freq_Y>0)).topography_fft
#quater 1 = freq_X>0 & freq_Y>0
#quater 2 = freq_X<0 & freq_Y>0
# esitmated angle = 45
# esitmated angle + 30, -30 range 
# q0 * rage =[ 0.5, 2]
grid_topo_crop_fft_peak_area

grid_topo_crop_fft_peak_area.plot()
# peak detection 
fft_peak = skimage.feature.peak_local_max(np.log(grid_topo_crop_fft_peak_area.fillna(0)).values, min_distance= 3)
fft_peak

if np.size(fft_peak)!=0: 
    grid_topo_crop.v_xr.loc[dict (crop=crop_i)]  = fft_peak[0]
    grid_topo_crop.u_xr.loc[dict (crop=crop_i)] = fft_peak[1]
else: 
    grid_topo_crop.v_xr.loc[dict (crop=crop_i)]  = np.nan
    grid_topo_crop.u_xr.loc[dict (crop=crop_i)] = np.nan

grid_topo_crop = grid_topo_crop.where(grid_topo_crop.crop < 10, drop =True)
grid_topo_crop
...
for crop_i in range(len(grid_topo_crop.crop)):
    cropped_data_fft = twoD_FFT_xr(grid_topo_crop.isel(crop=crop_i).dropna(dim="X", how="all").dropna(dim="Y", how="all").drop_vars(['u_xr','v_xr']))
    #cropped_data_fft_list.append(cropped_data_fft)
    distancefromcenter = np.sqrt(np.power(cropped_data_fft.freq_X, 2) + np.power(cropped_data_fft.freq_Y, 2))
    tangent = np.arctan( cropped_data_fft.freq_Y/cropped_data_fft.freq_X)
    #filtering 기준 설정 
    cropped_data_fft['distancefromcenter']=distancefromcenter
    cropped_data_fft['tangent']=tangent
    # peak searching range setup 
    cropped_data_fft_area= cropped_data_fft.where(
        (cropped_data_fft.distancefromcenter> grid_topo_crop_fft.ref_q0 *0.5)&
        (cropped_data_fft.distancefromcenter< grid_topo_crop_fft.ref_q0 *2)&
        (cropped_data_fft.tangent> 15*math.pi/180)&
        (cropped_data_fft.tangent< 75*math.pi/180)&
    ...
    # peak detection 
    fft_peak = skimage.feature.peak_local_max(np.log(cropped_data_fft_area.fillna(0)).values, min_distance= 2)
    # peak point 제대로 찾기 위해서  fillna(0) 와 np.log가 필요함. 
    if np.size(fft_peak)!=0: 
        grid_topo_crop.v_xr.loc[dict (crop=crop_i)]  = fft_peak[0][0]
        grid_topo_crop.u_xr.loc[dict (crop=crop_i)] = fft_peak[0][1]
    else: 
        grid_topo_crop.v_xr.loc[dict (crop=crop_i)]  = np.nan
        grid_topo_crop.u_xr.loc[dict (crop=crop_i)] = np.nan
...
# +
grid_topo["u_xr"]= grid_topo.topography.copy()
grid_topo["v_xr"]= grid_topo.topography.copy()

grid_topo["u_xr"].values = grid_topo_crop.u_xr.values.reshape(grid_topo.topography.values.shape)
grid_topo["v_xr"].values = grid_topo_crop.v_xr.values.reshape(grid_topo.topography.values.shape)
grid_topo["uv_xr_modulous"] = np.sqrt(np.power(grid_topo.u_xr, 2) + np.power(grid_topo.v_xr, 2))

grid_topo["u_xr_n"] = grid_topo.u_xr/grid_topo.uv_xr_modulous
grid_topo["v_xr_n"] = grid_topo.v_xr/grid_topo.uv_xr_modulous
grid_topo
# -

grid_topo.uv_xr_modulous.plot()

grid_topo['v_xr_n_neg']= grid_topo.v_xr_n*-1

#grid_topo.plot.streamplot(x="X", y="Y",u="u_xr",v="v_xr")
#grid_topo.where(grid_topo.X<2E-9,drop=True).where(grid_topo.Y<2E-9,drop=True).plot.quiver(x="X", y="Y",u="u_xr",v="v_xr")
grid_topo.where(grid_topo.X<2E-9,drop=True).where(grid_topo.Y<2E-9,drop=True).plot.quiver(x="X", y="Y",u="u_xr_n",v="v_xr_n")

# +
# rotate 90deg & normalized 

grid_topo.where(grid_topo.X<2E-9,drop=True).where(grid_topo.Y<2E-9,drop=True).plot.quiver(x="X", y="Y",u="v_xr_n_neg",v="u_xr_n")

# +
fig,ax = plt.subplots()
ax.streamplot(x=grid_topo.X.values*1E9,
              y=grid_topo.Y.values*1E9,
              u=grid_topo.v_xr_n_neg.values,
              v=grid_topo.u_xr_n.values )
plt.show()
...
grid_topo.where(grid_topo.X<2E-9,drop=True).where(grid_topo.Y<2E-9,drop=True).plot.streamplot(x="X", y="Y",u="v_xr_n_neg",v="u_xr_n", density=1, color='k')

grid_topo.v_xr_n_neg.values

grid_topo.v_xr_n_neg.values.shape
...
# Define the magnetic field data ATField and BTField

# Define the initial conditions
x0 = np.array([128, 160])
tA = np.array([-96.5, 160.5])
tB = np.array([-129.5, 130])


#  interpolation함수 구하고 
interAx = RectBivariateSpline(range(grid_topo.X.size), range(grid_topo.Y.size), grid_topo.v_xr_n_neg.values)
interAy = RectBivariateSpline(range(grid_topo.X.size), range(grid_topo.Y.size), grid_topo.u_xr_n.values)                            

                           
# Create interpolating functions for magnetic fields
#interAx = RectBivariateSpline(range(ATField.shape[0]), range(ATField.shape[1]), ATField[:, :, 0])
#interAy = RectBivariateSpline(range(ATField.shape[0]), range(ATField.shape[1]), ATField[:, :, 1])
#interBx = RectBivariateSpline(range(BTField.shape[0]), range(BTField.shape[1]), BTField[:, :, 0])
#interBy = RectBivariateSpline(range(BTField.shape[0]), range(BTField.shape[1]), BTField[:, :, 1])


# 도함수를 지정후 
# Define the ODEs for the motion
def systemA(X, t):
    x, y = X
    xdot = interAy(y, x, grid=False)
    ydot = interAx(y, x, grid=False)
    return [xdot, ydot]
'''
def systemB(X, t):
    x, y = X
...
#주어진 간격에대하여 
# Integrate the ODEs
t_span_A = np.linspace(tA[0], tA[1], 100)
#t_span_B = np.linspace(tB[0], tB[1], 100)
# 풀어낸다. 
solution_A = odeint(systemA, x0, t_span_A)
#solution_B = odeint(systemB, x0, t_span_B)
# -

solution_A.shape

# Plot the results
plt.imshow(grid_topo.topography.values, origin='upper', extent=(1, 256, 1, 256), cmap='viridis', aspect='auto', vmin=-0.1, vmax=0.02)
plt.plot(solution_A[:, 0], solution_A[:, 1], 'r', label='A')
#plt.plot(solution_B[:, 0], solution_B[:, 1], 'b', label='B')
plt.text(132, 243, 'A', color='red', fontsize=16)
#plt.text(243, 150, 'B', color='blue', fontsize=16)
plt.text(x0[0] + 14, x0[1] + 8, 'x0', color='black', weight='bold', fontsize=16)
plt.scatter(x0[0], x0[1], c='black', s=50)
plt.legend()
plt.xlabel('X')
plt.ylabel('Y')
plt.show()
...
# 데이터를 32x32 크기로 중심을 기준으로 자르고 리스트에 저장
for i in range(data.sizes["X"]):
    for j in range(data.sizes["Y"]):
        x_center = i
        y_center = j
        ...
        cropped_data = data.isel(X=slice(x_start, x_end), Y=slice(y_start, y_end))
        cropped_data_list.append(cropped_data)
# 리스트를 xarray 데이터 배열로 변환
cropped_data_array = xr.concat(cropped_data_list, dim="crop")
#cropped_data_array.topography.isel(crop=0).plot()
...
for i in range(data.sizes["X"]):
    for j in range(data.sizes["Y"]):
        x_center = i - crop_size // 2
        y_center = j - crop_size // 2
        if x_center >= 0 and y_center >= 0 and x_center + crop_size <= data.sizes["X"] and y_center + crop_size <= data.sizes["Y"]:
            cropped_data = data.isel(X=slice(x_center - crop_size, x_center + crop_size), Y=slice(y_center- crop_size, y_center + crop_size))
            cropped_data_list.append(cropped_data)
...
cropped_data_array.topography.isel(crop=1500).plot()
...
bias_mV_slices = np.arange(-10,10.1,0.5)
...
# # find peaks for all data pixel * apply k-mean clustering 
#

grid_LDOS_sg_pk = find_peaks_xr(grid_LDOS_sg)
grid_LDOS_sg_pk

grid_LDOS_sg_pk_pad  = peak_pad (grid_LDOS_sg_pk)
grid_LDOS_sg_pk_pad 

grid_LDOS_sg_pk_pad

# +
kmean_df = grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad.to_dataframe().reset_index()
#################
##  Classify peaks by using k-mean clustering 
####################
...
y_kmeans = kmeans.predict(X)
kmean_df['y_kmeans']=y_kmeans

grid_LDOS_sg_pk_pad['LDOS_fb_peaks_pad_kmean_y'] =  grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad.copy()
#grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad_kmean_y.shape
grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad_kmean_y.values = kmean_df.y_kmeans.values.reshape(grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad_kmean_y.shape)
# add kmean results
grid_LDOS_sg_pk_pad


# +
# kmean clustering 결과 따로 저장하기 

# +

grid_LDOS_sg_pk_pad['LDOS_fb_peak_kmeans']= grid_LDOS_sg_pk_pad.LDOS_fb.copy()
grid_LDOS_sg_pk_pad['LDOS_fb_peak_values']= grid_LDOS_sg_pk_pad.LDOS_fb.copy()


peak_kmeans= np.empty_like(grid_LDOS_sg_pk_pad.LDOS_fb_peak_values.values)
# isel 로 는 xarray 의 값 수정이 안되는 문제. 
# 각각  point 를 불러와서 수정하니까 안된다. 
# 전체 arrary를 만들어서 
# channel 전체를 바꿔줘야함. 

for x in range(grid_LDOS_sg_pk_pad.sizes['X']):
    for y in range(grid_LDOS_sg_pk_pad.sizes['Y']):
            #  np.argwhere(grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad.isel(X=x, Y=y).values==bias_i   bias_i 가 for loop를 돌때 그값이 LDOS_fb_peaks_pad 안에 있으면. 
            #  LDOS_fb_peaks_pad 에서 해당하는 kmean 값을 불러온다. (index를 기준으로) 
            #  bias_i 에서 없으면  0 를 반환 
            bias_mV_values = np.array (
                [ grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y).values.astype(int)[np.argwhere(grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad.isel(X=x, Y=y).values==bias_i)][0][0]
                 if bias_i in grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad.isel(X=x, Y=y).values.astype(int) 
                 else 0 
                 for bias_i, LDOS_fb_v in enumerate (grid_LDOS_sg_pk_pad.isel(X=x, Y=y).LDOS_fb_peak_kmeans) ])
            
            
            
            # bias_mV 를 돌렴ㄴ서 
            peak_kmeans[x,y,:] = bias_mV_values
        
grid_LDOS_sg_pk_pad.LDOS_fb_peak_kmeans.values = peak_kmeans
        # x,y 에 bias_mV 가운데 peak 에 해당하는 부분만 남기고 나머지는 0로 

# +
#grid_LDOS_sg_pk_pad.LDOS_fb_peak_values

np.save('grid_LDOS_sg_pk_pad_kmean.npy', grid_LDOS_sg_pk_pad.LDOS_fb_peak_values)

# +
# 이번에는 peak 이 있는 곳의 LDOS 값들만 따로 저장하기 
...
peak_LDOS= np.empty_like(grid_LDOS_sg_pk_pad.LDOS_fb_peak_values.values)
# isel 로 는 xarray 의 값 수정이 안되는 문제. 
# 각각  point 를 불러와서 수정하니까 안된다. 
# 전체 arrary를 만들어서 
# channel 전체를 바꿔줘야함. 
...
for x in range(grid_LDOS_sg_pk_pad.sizes['X']):
    for y in range(grid_LDOS_sg_pk_pad.sizes['Y']):
            # 이번에는 해당하는 LDOS 에서  bias_i  에 해당하는 값을 저장한다. 
            LDOS_fb_values = np.array (
                [ grid_LDOS_sg_pk_pad.LDOS_fb.isel(X=x, Y=y, bias_mV = bias_i)
                 if bias_i in grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad.isel(X=x, Y=y).values.astype(int) 
                 else 0 
                 for bias_i, LDOS_fb_v in enumerate (grid_LDOS_sg_pk_pad.isel(X=x, Y=y).LDOS_fb_peak_kmeans) ])
            # bias_mV 를 돌렴ㄴ서 
            peak_LDOS[x,y,:] = LDOS_fb_values
        
grid_LDOS_sg_pk_pad.LDOS_fb_peak_values.values = peak_LDOS
        # x,y 에 bias_mV 가운데 peak 에 해당하는 부분만 남기고 나머지는 0로 
# -

np.save('LDOS_fb_peak_values.npy', grid_LDOS_sg_pk_pad.LDOS_fb_peak_values)


        test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y).values.astype(int)[][0][0]         
        test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y).values.astype(int)[np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values==bias_i)][0][0] 

np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=1, Y=1).values==7)

# +

for x in range(test_xr.sizes['X']):
    for y in range(test_xr.sizes['Y']):
        bias_mV_values = np.array ([ test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y).values.astype(int)[np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values==bias_i)][0][0] if bias_i in test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values.astype(int) else 0 for bias_i, LDOS_peak_km in enumerate (test_xr.isel(X=x, Y=y).LDOS_fb_peak_kmeans) ])
        # bias_mV 를 돌렴ㄴ서 
        peak_kmeans[x,y,:] = bias_mV_values
        
test_xr.LDOS_fb_peak_values.values = peak_kmeans

# +
#[print (x) for x in range(len(grid_LDOS_sg_pk_pad.X)) ]
# -

test_xr = grid_LDOS_sg_pk_pad.where(grid_LDOS_sg_pk_pad.X<0.5E-9,drop =True).where(grid_LDOS_sg_pk_pad.Y<0.5E-9,drop =True)
test_xr

test_xr['LDOS_fb_peak_kmeans']= test_xr.LDOS_fb.copy()
test_xr['LDOS_fb_peak_values']= test_xr.LDOS_fb.copy()
test_xr

# +
peak_kmeans= np.empty_like(test_xr.LDOS_fb_peak_values.values)
# isel 로 는 xarray 의 값 수정이 안되는 문제. 
# 각각  point 를 불러와서 수정하니까 안된다. 
# 전체 arrary를 만들어서 
# channel 전체를 바꿔줘야함. 
...
for x in range(test_xr.sizes['X']):
    for y in range(test_xr.sizes['Y']):
        bias_mV_values = np.array ([ test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y).values.astype(int)[np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values==bias_i)][0][0] if bias_i in test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values.astype(int) else 0 for bias_i, LDOS_fb_v in enumerate (test_xr.isel(X=x, Y=y).LDOS_fb_peak_kmeans) ])
        # bias_mV 를 돌렴ㄴ서 
        peak_kmeans[x,y,:] = bias_mV_values
...
np.array ([ test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y).values.astype(int)[np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values==bias_i)][0][0] if bias_i in test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values.astype(int) else 0 for bias_i, LDOS_peak_km in enumerate (test_xr.isel(X=x, Y=y).LDOS_fb_peak_kmeans) ]).shape

test_xr.LDOS_fb_peak_kmeans.isel(X=0, Y=0).values.shape
#test_xr.LDOS_fb_peak_kmeans.isel(X=0, Y=0,bias_mV=0).values

test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y).values.astype(int)[np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values==7)][0][0]

test_xr.X.isel(X=x)

test_xr.LDOS_fb_peak_values.loc[{'X':test_xr.X.isel(X=x), 'Y':test_xr.Y.isel(Y=y)}]

# +
#test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y, peaks = peak_indx).values

np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values  == 1)

[np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values==bias_i)]

# +
x=1
y=1
xy11test = np.array([ test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y).values.astype(int)[np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values==bias_i)][0][0] if bias_i in test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values.astype(int) else np.nan for bias_i, LDOS_peak_km in enumerate (test_xr.isel(X=x, Y=y).LDOS_fb_peak_kmeans) ])

xy11test
# -

test_xr.LDOS_fb_peak_kmeans.isel(X=1, Y=1).values = xy11test
test_xr.LDOS_fb_peak_kmeans.isel(X=1, Y=1)

test_xr.LDOS_fb_peak_kmeans

for x in range(test_xr.sizes['X']):
    for y in range(test_xr.sizes['Y']):
        print(test_xr.LDOS_fb_peak_kmeans.loc[{"X":2, "Y": 2}] )

for x in range(test_xr.sizes['X']):
    for y in range(test_xr.sizes['Y']):
        test_xr.LDOS_fb_peak_kmeans.loc[{"X":x, "Y": y}] = [ test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y).values.astype(int)[np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values==bias_i)][0][0] if bias_i in test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values.astype(int) else 0 for bias_i, LDOS_peak_km in enumerate (test_xr.isel(X=x, Y=y).LDOS_fb_peak_kmeans) ]
...
for x in range(test_xr.sizes['X']):
    for y in range(test_xr.sizes['Y']):
        ...
        for bias_mV_i, bias_mV in enumerate(test_xr.bias_mV):
            if bias_mV_i in test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).astype(int):
                print('bias_mV_i',bias_mV_i)
                peak_indx  = np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values  == bias_mV_i)[0][0]
                print ('peak_indx',peak_indx)
                print(test_xr.LDOS_fb_peak.isel(X=x, Y=y, bias_mV = bias_mV_i).values)
                print(test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y, peaks = peak_indx).values)
                test_xr.LDOS_fb_peak.loc[{'X':x, 'Y':y, 'bias_mV':bias_mV_i}]= test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y, peaks = peak_indx).values
                print(test_xr.LDOS_fb_peak.isel(X=x, Y=y, bias_mV = bias_mV_i).values )
            else :
                # Update a specific location to np.nan using the loc attribute
                test_xr.LDOS_fb_peak.loc[{'X':x, 'Y':y, 'bias_mV':bias_mV_i}] = np.nan                    #print('x',x,'y',y,'bias_mV',bias_mV_i)
test_xr.LDOS_fb_peak                    

for bias_mV_i, bias_mV in enumerate(test_xr.bias_mV):
    for y in  range(len(test_xr.Y)):
        for x in range(len(test_xr.X)):
                if bias_mV_i in test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).astype(int):
                    #print ('bias_mV_i', bias_mV_i)
                    peak_indx  = np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).values  == bias_mV_i)[0][0]
                    test_xr.LDOS_fb_peak.isel(X=x, Y=y, bias_mV = bias_mV_i).values = test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=x, Y=y, peaks = peak_indx).values
                    #print('peak_indx', peak_indx)
                else :
                    test_xr.LDOS_fb_peak.isel(X=x, Y=y, bias_mV = bias_mV_i).values = np.nan
                    #print('x',x,'y',y,'bias_mV',bias_mV_i)
test_xr.LDOS_fb_peak                    

test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=1, Y=1, peaks = 1).values

test_xr.LDOS_fb_peaks_pad.isel(X=1, Y=1).astype(int)
np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=1, Y=1).values  == 160)[0][0]

#np.argwhere(test_xr.LDOS_fb_peaks_pad.isel(X=1, Y=1).values  == 1)
test_xr.LDOS_fb_peaks_pad_kmean_y.isel(X=1, Y=1, peaks = 1)

# +
for bias_mV_i, bias_mV in enumerate(test_xr.bias_mV):
    bias_slice =  test_xr.LDOS_fb.isel(bias_mV = bias_mV_i)
    
    bais_mV_peaks =  np.array([test_xr.LDOS_fb.isel(X=x, Y=y, bias_mV = bias_mV_i)
    if bias_mV_i in test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).astype(int) else np.nan for x in range(len(test_xr.X)) for y in  range(len(test_xr.Y)) ])
    #bais_mV_peaks = bais_mV_peaks.reshape(bias_slice.shape)
    test_xr.bias_peaks.isel(bias_mV =bias_mV_i).values  = bais_mV_peaks
...
for bias_mV_i, bias_mV in enumerate(test_xr.bias_mV):
    bais_mV_peaks = [test_xr.LDOS_fb.isel(X=x, Y=y, bias_mV = bias_mV_i) if bias_mV_i in test_xr.LDOS_fb_peaks_pad.isel(X=x, Y=y).astype(int) else np.nan for x in range(len(test_xr.X)) for y in  range(len(test_xr.Y)) ]

test_xr.LDOS_fb_peaks_pad.isel(X=11, Y=11).astype(int)
#bais_mV_peaks

test_xr.LDOS_fb_peak.to_dataframe().isna().sum()

    
    in test_xr.LDOS_fb_peaks_pad.isel(X=0, Y=0) else np.nan
      
    print(bias_mV) if bias_mV_i is in  test_xr.LDOS_fb_peaks_pad.isel(X=0, Y=0)
    result = [bias_mV if bias_mV_i in test_xr.LDOS_fb_peaks_pad.isel(X=0, Y=0) else np.nan ]

    #test_xr.LDOS_fb_peaks_pad.isel(X=0, Y=0)
#test_xr.bias_mV

test_xr.LDOS_fb_peaks_pad.isel(X=0, Y=0).values

# +
 test_xr.bias_mV [test_xr.LDOS_fb_peaks_pad.isel(X=0, Y=0).values.astype(int)]
...
# +
grid_LDOS_sg_pk_pad.isel( X=((bias_mV= 1)

for bias_mV_i in range (len(grid_LDOS_sg_pk_pad.bias_mV)):
    [bias_mV_i in grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad.isel(X=x, Y=y) for x in range(len(grid_LDOS_sg_pk_pad.X)) for y in 
     range(len(grid_LDOS_sg_pk_pad.Y)) ]
# -

[grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad.isel(X=x, Y=y) for x in range(len(grid_LDOS_sg_pk_pad.X)) for y in range(len(grid_LDOS_sg_pk_pad.Y)) ]


# +
test_xr = grid_LDOS_sg_pk_pad.LDOS_fb
# _peaks_pad 에 있으면, 값을 그대로, 아니면 np.nan으로 

for bias_mV_i  in grid_LDOS_sg_pk_pad.bias_mV:
    for 
    
    bias_mV_i in  LDOS_fb_peaks_pad 
    for test_xr_xybias_mV  in test_xr_xy for test_xr_xy in test_xr_y for test_xr_y in grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad]


#in grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad
# -

grid_LDOS_sg_pk_pad.bias_mV

#grid_LDOS_sg_pk_pad.data_vars
grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad          
#find_peaks_prominence_xr(grid_LDOS_sg_pk_pad)

test_df = grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad.to_dataframe().reset_index()
...
grid_LDOS_sg_pk_pad['LDOS_fb_peaks_pad_kmean_y'] =  grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad.copy()
#grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad_kmean_y.shape
grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad_kmean_y.values = test_df.y_kmeans.values.reshape(grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad_kmean_y.shape)
grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad_kmean_y
...
grid_LDOS_sg_pk_pad.LDOS_fb_peaks_pad_kmean_y.isel(X =1).plot()
plt.show()
...
def local_FFT_A_uv_xr(grid_topo_crop, ref_a0=0.38E-9, estimated_q0_range=(0.5, 2), angle_margin=20, peak_angle_deg=45, quadrant=None):
    ...
    # Initialize arrays for A_u_xr and A_v_xr
    grid_topo_crop['A_u_xr'] = grid_topo_crop.topography.mean(dim=["X", "Y"]).copy()
    grid_topo_crop['A_v_xr'] = grid_topo_crop.topography.mean(dim=["X", "Y"]).copy()
    ...
    for crop_i in range(len(grid_topo_crop.crop)):
        # Perform 2D FFT on the cropped data
        cropped_data_fft = twoD_FFT_xr(grid_topo_crop.isel(crop=crop_i).dropna(dim="X", how="all").dropna(dim="Y", how="all").drop_vars(['A_u_xr', 'A_v_xr']))
...
for crop_i in range(len(grid_topo_crop.crop)):
    ...
    # peak searching range setup 
    cropped_data_fft_area= cropped_data_fft.where(
        (cropped_data_fft.distancefromcenter> grid_topo_crop.ref_q0 *0.5)&
        (cropped_data_fft.distancefromcenter< grid_topo_crop.ref_q0 *2)&
        (cropped_data_fft.tangent> 15*math.pi/180)&
        (cropped_data_fft.tangent< 75*math.pi/180)&
        (cropped_data_fft.freq_X>0)&
        (cropped_data_fft.freq_Y>0)).topography_fft
    ...
    #quater 2 = freq_X<0 & freq_Y>0
    # esitmated angle = 45
    # esitmated angle margine  + 30, -30 range 
    # q0 * rage =[ 0.5, 2]
