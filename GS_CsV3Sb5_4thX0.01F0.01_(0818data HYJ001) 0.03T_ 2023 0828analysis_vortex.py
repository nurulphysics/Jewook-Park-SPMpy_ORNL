...
#
# ## <font color= red > Magnetic field 0.03 T (Z)   </font>
# -
...
# line data
grid_xr = grid2xr(files_df[files_df.type=='3ds'].file_name.iloc[8])
grid_xr
...
#hv_XY_slicing(grid_LDOS, ch = 'LDOS_fb',slicing= 'Y',frame_width=300).opts(clim=(0, 0.6E-9)) # check low intensity area
...
bound_box = hv.streams.BoundsXY(source = bbox_points,
                                bounds=(0,0,0,0))
dmap.opts(clim = (0,0.5E-9))*bbox_points
...
rec_in_topo_bb3 =  patches.Rectangle( rec_xy_bb3 , rec_w_px_bb3,rec_h_px_bb3 , linewidth=2, edgecolor=sns.color_palette('tab10')[2], facecolor='none')
rec_in_topo_bb4 =  patches.Rectangle( rec_xy_bb4 , rec_w_px_bb4,rec_h_px_bb4 , linewidth=2, edgecolor=sns.color_palette('tab10')[3], facecolor='none')
...
bias_mV_slices = np.arange(-1.00,1.01,0.2)
...
bound_box = hv.streams.BoundsXY(source = bbox_points,
                                bounds=(0,0,0,0))
dmap.opts(clim = (0,0.6E-9))*bbox_points
#dmap*bbox_points
...
SCgaps_posD1 =r'+$\Delta_{1}$ = '+ str(round (
    grid_LDOS_bbox_pk_df[grid_LDOS_bbox_pk_df.y_kmeans  ==2].mean().bias_mV,
    2) ) +r'$\pm$'  +str(round (
    grid_LDOS_bbox_pk_df[grid_LDOS_bbox_pk_df.y_kmeans  ==2].std().bias_mV,
    2) )    +' mV'
